...
// ==========================================================================
const AUTH_KEY = 'coachboard_auth_user';
...
function handleSignIn(e){
  ...
  const keepSigned = document.getElementById('check')?.checked;

  const username = (userInput?.value || '').trim();
  const password = (passInput?.value || '').trim();

  if(!username || !password){
    showToast('Vul alstublieft een gebruikersnaam en wachtwoord in.', 'warning');
    return;
  }

  // Save session
  const userData = JSON.stringify({ user: username, loggedInAt: new Date().toISOString() });
  if(keepSigned){
  ...
  showToast(`Welkom terug, ${username}!`, 'success');
}
...
function handleSignUp(e){
  ...
  const passRepeat = document.getElementById('signupPassRepeat');
  const emailInput = document.getElementById('signupEmail');

  const username = (userInput?.value || '').trim();
  const password = (passInput?.value || '').trim();
  const repeat = (passRepeat?.value || '').trim();
  ...
  if(password !== repeat){
    showToast('De ingevulde wachtwoorden komen niet overeen.', 'error');
    return;
  }

  // Successful signup & login
  const userData = JSON.stringify({ user: username, email, loggedInAt: new Date().toISOString() });
  localStorage.setItem(AUTH_KEY, userData);
  ...
  showToast(`Account aangemaakt! Welkom, ${username}.`, 'success');
}
